...
def task_pkg_create_cmd() -> tuple[int, str, str]:
    ...
    if (pkg := (src_dir / 'ros_pkg')).exists():
        shutil.rmtree(pkg)

    cmd = "ros2 pkg create ros_pkg --build-type ament_python --license MIT --dependencies rclpy std_msgs --maintainer-name kcci --maintainer-email kcci@kcci.com --description \"D015 homework ROS2 workspace\""
...
def task_docker_run_cmd() -> str:
    ...
    # TDO: 여기에 코드를 작성하시오
    return (
        "docker run -it --rm --name humble-gui --network=host -e DISPLAY=$DISPLAY -e ROS_DOMAIN_ID=24 -e RMW_IMPLEMENTATION=rmw_cyclonedds_cpp -v /tmp/.X11-unix:/tmp/.X11-unix:rw arm64v8/ros:humble bash"
    )
